...
- [ ] **Manual QA on staging** — spot-check:
  - [ ] Login with email + password works (Supabase auth)
  - [ ] **Multi-user Realtime:** two browsers — User A edits order, User B dashboard updates without refresh
...
- [ ] **R2 Edge Functions**: confirm all 4 functions (`r2-presign-upload`, `r2-presign-download`, `r2-confirm-upload`, `r2-delete`) are deployed to production Supabase project
  ...
  ```
- [ ] **Database migrations**: confirm all migrations are applied to production DB, including:
  ...
  npm run audit:staff   # optional: verify profiles vs legacy config.personnel
  ```
...
- [ ] **Vercel** (git-push deploy): `vercel.json` runs `bash deploy/build.sh` — env vars in **Vercel → Settings → Environment Variables**:
  - `VITE_SUPABASE_URL` (or `PULSE_SUPABASE_URL`)
  - `VITE_SUPABASE_ANON_KEY` = **anon public** key only (`eyJ...`) — **not** service_role / `sb_secret_...`
...
- [ ] Browser console shows `[Pulse] Storage backend: Supabase → …` and `pulseUsesSupabaseStorage()` returns `true`
- [ ] `js/pulse-config.js` has production Supabase URL — **`js/pulse-config.local.js`** is generated at deploy (or empty locally)
- [ ] Log in with **email + password** (Supabase Auth) — each role routes correctly:
