...
## Production status (June 2026)

Pulse production uses **`PULSE_STORAGE_BACKEND = 'supabase'`**. All business data is in Postgres; IndexedDB is not used in production. Personnel and login come from **`profiles`** + Supabase Auth (email/password). See [`production-status.md`](production-status.md) for verification steps and npm scripts.
...
| steps | JSONB | See [`product-workflow-config.md`](../app/product-workflow-config.md) |
