-- Reject duplicate login emails when adding personnel (email = first-name@bazaar-admin.com).
-- When editing, allow the same profile to keep its email; block if another account uses it.

DROP FUNCTION IF EXISTS public.upsert_pulse_personnel(TEXT, TEXT, TEXT, TEXT, BOOLEAN);
