...
CREATE INDEX IF NOT EXISTS workflow_override_log_order_id_idx ON workflow_override_log(order_id);
CREATE INDEX IF NOT EXISTS workflow_override_log_changed_at_idx ON workflow_override_log(changed_at DESC);

ALTER TABLE workflow_override_log ENABLE ROW LEVEL SECURITY;

-- Idempotent Policy Resets
DROP POLICY IF EXISTS "workflow_override_log_select" ON workflow_override_log;
CREATE POLICY "workflow_override_log_select"
  ON workflow_override_log FOR SELECT
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "workflow_override_log_insert" ON workflow_override_log;
